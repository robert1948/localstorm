...
from app.schemas_enhanced import (
    ...
    EmailVerificationRequest, EmailVerificationCode, EmailVerificationResponse
)
...
async def send_verification_code(
    ...
    try:
        ...
        if request.purpose == "login":
            user = db.query(UserV2).filter(UserV2.email == request.email).first()
            if not user:
        ...
        auth_service.log_event(
            db, None, "verification_code_error",
            f"Failed to send verification code to {request.email}",
            success=False, **client_info, endpoint="/api/auth/send-verification-code",
            error_message=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def login_with_verification_code(
    ...
    try:
        ...
        code_valid, message = await email_verification_service.verify_login_code(
            db=db,
            email=verification.email,
            code=verification.code
        )
        ...
        if not code_valid:
            auth_service.log_event(
                db, None, "login_attempt",
                f"Failed code-based login for {verification.email}: {message}",
                success=False, **client_info, endpoint="/api/auth/login-with-code",
        ...
        # Get user by email
        user = db.query(UserV2).filter(UserV2.email == verification.email).first()
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="User not found"
        ...
        # Log successful login
        auth_service.log_event(
            db, user.id, "login_with_code",
            f"User logged in with verification code: {user.email}",
            success=True, **client_info, endpoint="/api/auth/login-with-code"
        )
        ...
        auth_service.log_event(
            db, None, "login_error",
            f"Code-based login error for {verification.email}",
            success=False, **client_info, endpoint="/api/auth/login-with-code",
            error_message=str(e)
        )
        raise HTTPException(
