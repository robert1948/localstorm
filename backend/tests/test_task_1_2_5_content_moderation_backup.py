...
class TestContentModerationIntegration:
    ...
    # def test_ai_prompt_with_content_moderation(self, mock_user, mock_redis, mock_ai_service, client):
        """Test AI prompt endpoint with content moderation"""
        # Mock user
        ...
        # Mock AI service to return content that needs moderation
        mock_ai_service.return_value = {
            "response": "You should take this medication for your illness without consulting a doctor.",
            "suggestions": ["Get medical help"],
            "actions": [{"text": "Find Doctor", "action": "/doctors"}],
            "context": {}
        }
        ...
        # Response should include disclaimer due to medical content
        assert "response" in data
        assert "moderation_applied" in data
        # Check if medical disclaimer was added
        if "Disclaimer" in data["response"]:
            assert "not medical advice" in data["response"].lower()
